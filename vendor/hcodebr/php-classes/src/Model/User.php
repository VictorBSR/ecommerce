...
class User extends Model {
    ...
    public static function getForgot($email, $inadmin = true) {
        ...
        if (count($results) === 0) {
            ...
            if (count($results2) === 0) {
                ...
                $mailer = new Mailer($data["desemail"], $data["desperson"], "Redefinição de senha da Hcode Store", "forgot",
                array(
                    "name"=>$data["desperson"],
                    "link"=>$link
                )    
    ...
    public static function setForgotUsed($idrecovery) {
        ...
        $sql = new Sql();
        $sql->query("UPDATE ub_userspasswordsrecoveries SET dtrecovery = NOW() WHERE idrecovery = :idrecovery", array (
            ":idrecovery"=>$idrecovery
        ));
